...
function getBabyOutfit(baby) {
    return `${baby.name} is wearing a ${baby.outfit.shirt} shirt, ${baby.outfit.pants} pants, and ${baby.outfit.shoes}`;
}

function feedBaby(baby) {
    // Shuffle favoriteFoods array
    let shuffledFoods = baby.favoriteFoods.sort(() => Math.random() - 0.5);
